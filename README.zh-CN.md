...
- **免按模式** —— 点一下第二个快捷键开始，再点一下结束，适合长段口述。忘了点第二下也没关系：房间安静够久之后这一段会自己结束（默认 30 秒，设置里可选 15/30/60 秒或关掉）。「安静」是拿你当下这个房间当基准量出来的，不是一个固定音量，所以背景有风扇、扫地机器人也不会让它永远停不下来 —— 只要你说话的音量大概是背景的两倍以上就行。全程没人说话的那种，直接丢掉，不送去转写。
- **Esc 取消** —— 说到一半改主意，或者发现焦点在错的窗口？按 <kbd>esc</kbd> 就把这次口述丢掉：录音阶段丢音频，转写或清理阶段丢文字，什么都不会落到光标上。只有真的在口述的时候 esc 才被占用，其他时候它照常工作。
...
要说长一点就改用 <kbd>⇧</kbd><kbd>⌘</kbd><kbd>K</kbd>：点一下开始录，再点一下停，或者等它自己因为安静而停下。文字落下之前的任何时候，<kbd>esc</kbd> 都可以把这次口述丢掉。
...
| `SilenceWatchdog.swift` | 免按模式安静够久就自己收尾 |
...
免按模式的自动收尾也有自己的无界面检查：用合成的音量轨迹跑一遍判断逻辑，重点是那些「不该停」的情形（声音小的人、长时间思考的停顿、吵到没法判断的房间）：
...
| [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md) | 装到另一台 Mac、公证、为什么只用 GitHub Releases |
| [docs/RELEASING.md](docs/RELEASING.md) | 发版流程一步步怎么走，以及踩过的坑 |
| [docs/FEEDBACK.md](docs/FEEDBACK.md) | 反馈 bug 或崩溃报告、报告去哪里、如何符号化崩溃日志 |

## 更新检查

菜单栏的 **Check for updates…** 会一次检查两样东西，并分别告诉你结果：

- **App 本身** —— 问 GitHub 有没有发布更新版本的 LocalFlow，可直接打开 release 页面。
  下载包已做公证（notarized），双击即可打开。
- **语音模型** —— 问 Hugging Face 自上次查看以来有没有新模型。

两者都不会自动安装。LocalFlow 没有自动更新器，App 更新是你自己决定下载运行；而全新的语音
模型也不能直接启用 —— 准确率因人声和语言而异，必须先做基准测试并加入选择列表。

**这一次点击是 LocalFlow 唯一联网的时刻。** 它不会定时运行，不会发送任何与你或你的听写内容
相关的信息，只在你主动点击时发生。音频和转写文本完全不离开你的 Mac。
